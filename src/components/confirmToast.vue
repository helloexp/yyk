...
export default {
  data () {
    return {
      ...
      titleText: '提示', // 提示框标题
      content: '请输入提示框内容', // 提示框的内容
      cancelText: '取消', // 取消按钮的文字
      confirmText: '确认', // 确认按钮的文字
      type: 'confirm', // 表明弹框的类型：confirm - 确认弹窗（有取消按钮）；alert - 通知弹框（没有取消按钮）
      outerData: null // 用于记录外部传进来的数据，也可以给外部监听userControl，事件的函数提供判断到底是哪个事件触发的
    }
  },
  methods: {
    show (content, config) {
      this.content = content || '请输入提示框内容'
    ...
    clickFun (type) {
      this.$emit('userControl', type, this.outerData)
      this.hidden()
    }
  }
}
